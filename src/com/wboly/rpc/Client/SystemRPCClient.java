...
import com.wboly.rpc.service.SystemConfigService;
import com.wboly.system.sys.util.SpringUtil;

public class SystemRPCClient extends BaseRpcClient {
	private TMultiplexedProtocol tMultiplexedProtocol;

	public SystemConfigService.Client client;
	...
	public SystemRPCClient() {
		...
		TMultiplexedProtocol tmp = new TMultiplexedProtocol(protocol, "SystemConfigService");
		client = new SystemConfigService.Client(tmp);
		try {
			transport.open();
		...
		/*try {
			...
			TMultiplexedProtocol mp1 = new TMultiplexedProtocol(protocol, "SystemConfigService");
			SystemConfigService.Client client = new SystemConfigService.Client(mp1);
			this.client = client;
		} catch (TException x) {
			x.printStackTrace();
		}*/
