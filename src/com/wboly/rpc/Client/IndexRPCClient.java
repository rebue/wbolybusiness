...
import com.wboly.rpc.service.IndexService;
import com.wboly.system.sys.util.SpringUtil;

public class IndexRPCClient extends BaseRpcClient {
	...
	public IndexRPCClient() {
		...
		TMultiplexedProtocol tmp = new TMultiplexedProtocol(protocol, "IndexService");
		client = new IndexService.Client(tmp);
		try {
			transport.open();
		 ...
		 * TMultiplexedProtocol(protocol, "IndexService"); IndexService.Client client =
		 * new IndexService.Client(mp1); this.client = client; } catch (TException x) {
		 * x.printStackTrace(); }
		 */
	}
