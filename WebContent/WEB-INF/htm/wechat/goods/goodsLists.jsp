	...
	<div id="offCanvasWrapper" class="mui-off-canvas-wrap mui-draggable mui-slide-in">
		...
		<aside id="aside" class="mui-off-canvas-right">
			<div id="offCanvasSideScroll" class="mui-scroll-wrapper">
					...
					<ul class="mui-table-view">
						...
						<li class="mui-table-view-cell mui-row priceselect">
							<div class="mui-input-row mui-col-sm-5 mui-col-xs-5">
								<input type="number" class="mui-input-clear" placeholder="最低价" name="low">
							</div>
							<p class="mui-col-sm-2 mui-col-xs-2">──</p>
							<div class="mui-input-row mui-col-sm-5 mui-col-xs-5">
								<input type="number" class="mui-input-clear" placeholder="最高价" name="hign">
							</div>
						</li>
				...
				<div id="buttonbox" class="mui-row">
					<button class="mui-col-sm-6 mui-col-xs-6 filterbtn btn1" id="clearSelect">清除选项</button>
					<button class="mui-col-sm-6 mui-col-xs-6 filterbtn btn2" id="filterReady">确定</button>
				</div>
			</div>
		</aside>
		<!-- 主页面容器 -->
		<div class="mui-inner-wrap">
			<header class="mui-bar mui-bar-nav goods-list-head">
				...
				<span id="changeview" class="mui-icon mui-icon-list mui-pull-right wboly-listype"></span>
			</header>
			<div id="typetab"
				class="mui-segmented-control mui-segmented-control-inverted">
				<a class="mui-control-item" href="#item1mobile" data-type="0">综合 </a> 
				<a class="mui-control-item" href="#item3mobile" data-type="2" data-sort="0"> 价格 </a> 
				<a class="mui-control-item unactive" id="filter" href="#vitem3mobile">筛选<span class="mui-icon-extra mui-icon-extra-filter"></span></a>
			</div>
			<div id="refreshContainer" class="mui-scroll-wrapper" style="margin-top: 91px;">
	...
	<script type="text/javascript" charset="utf-8"> 
		...
		(function($, doc) {	
			...
			$.ready(function() {
				...
				// 返回指定的页面
				mui('.goods-list-head').on('tap','a',function(){document.location.href=this.href;});
				mui('#content').on('tap','a',function(){document.location.href=this.href;});
				mui('#shoppingcart').on('tap','a',function(){document.location.href=this.href;});
				...
				mui("#typetab").on('tap','a:not(.unactive)',function(){
				  ...
				  if(this.getAttribute("data-sort") == 0){//升降序标识，如果当前是降序，则转为升序
				  	...
				  	sort = 1;
				  }else if(this.getAttribute("data-sort") == 1){//升降序标识，如果当前是升序，则转为降序
				...
				mui("#buttonbox").on('tap','#filterReady',function(){
					...
					var obj = document.querySelectorAll(".brandlist input:checked");
		...
		function AjaxAppendData(){
			...
			start += 10;
			limit += 10;
			mui.ajax('${ctx}/wechat/goods/getGoodsList.htm',{ //测试用,模拟数据
				data:{
					key :'59c23bdde5603ef993cf03fe64e448f1',
					type : type,//排序方式标识
					sortType : sort,//升序降序标识
