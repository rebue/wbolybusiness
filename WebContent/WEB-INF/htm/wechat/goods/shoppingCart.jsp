...
<body>
	...
	<footer class="mui-bar total-box mui-checkbox mui-left" id="total-box">
		<label for="totalcheck">全选</label> <input name="totalcheckbox"
			value="0" type="checkbox" checked="checked" id="totalcheck">
	...
	<script type="text/javascript" charset="utf-8">
		...
		(function($, doc) {
			...
			$.ready(function() {
				...
				mui(document).on("change", "#goods-main-box .mui-input-row .mui-numbox input[type='number']", function(e) {
					...
					// 上线编号
					var onlineId = this.parentNode.parentNode.parentNode.parentNode
							.querySelector("input[name='onlineId']").value;
					// 上线规格编号
					var onlineSpecId = this.parentNode.parentNode.parentNode.parentNode
							.querySelector("input[name='onlineSpecId']").value;
					// 购物车编号
					var cartId = this.parentNode.parentNode.parentNode.parentNode
							.querySelector("input[name='cartId']").value;
					// 购物车数量
					var cartCount = this.parentNode.parentNode.parentNode.parentNode
							.querySelector("input[name='cartCount']").value;
					// 增加之后的购物车数量
					var addCartCount = this.parentNode.parentNode.parentNode.parentNode
				...
				doc.getElementById('goPay').addEventListener('tap', function(event) {
					...
					checkboxs.each(function(e) {
						...
						// 供应商id
						var supplierId = this.parentNode.querySelector("input[name='supplierId']").value;
						// 押货类型
						var subjectType = this.parentNode.querySelector("input[name='subjectType']").value;
						// 商品主图 
					...
					if (arr != null && arr.length > 0) {
						...
						console.log(num);
						var jsondata = JSON.parse('[{"info":[' + arr + '],"totalPrice":"' + totalPrice
										+ '","totalBack":"' + totalBack + '","totalNumber":"' + num + '"}]');
				...
				$('.mui-bar-nav').on('tap', '#editor', function(event) {
					...
					if (elem.classList.contains("on")) {
						...
						case_1.querySelectorAll(".total-price-box")[0].style.opacity = 1;
						case_2.style.top = 50 + "px";
					} else {
						elem.classList.add("on");
						elem.innerText = "退出编辑";
						case_1.style.opacity = 0;
						case_1.querySelectorAll(".total-price-box")[0].style.opacity = 0;
		...
		function compute() {
			...
			mui(".total-price-box h5 span")[0].innerHTML = "¥" + totalBack.toFixed(2);
