...
<body>
			...
			<div id="offCanvasWrapper_2" class="mui-off-canvas-wrap mui-slide-in">
				<aside id="aside_2" class="mui-off-canvas-right">
					<header class="mui-bar mui-bar-nav" id="list-head">
						<a class="mui-icon mui-icon-left-nav mui-pull-left" id="action_back_side_2"></a>
						<h1 class="mui-title">所在地区</h1>
					</header>
					<div class="mui-content">
						<div id='list' class="mui-indexed-list">
							<div class="mui-indexed-list-search mui-input-row mui-search">
								<input type="search" class="mui-input-clear mui-indexed-list-search-input" placeholder="搜索文字或拼音首字母" maxlength="10">
							</div>
							<div class="tab-bar mui-tab-bar" id="status-bar">
								<div id="status-bar-left"></div>
								<a class="tab-active" id="status-bar-right">选择省</a>
							</div>
				...
				<div class="mui-inner-wrap">
					<div class="mui-scroll">
						...
						<ul class="mui-table-view mui-input-group">
							<li class="mui-table-view-cell side-head"><a
								class="mui-icon mui-icon-left-nav mui-pull-left"
								id="action_back_side"></a> <span></span></li>
							<li class="mui-table-view-cell mui-input-row"><label>收货人：</label>
								<input type="text" value="" name="receiver" placeholder="建议填写真实姓名"></li>
							<li class="mui-table-view-cell mui-input-row">
								<a class="mui-navigate-right" id="AreaPicker"> <label>区域：</label>
									<span class="area-text" id="AreaName"></span>
								</a>
							</li>
							<li class="mui-table-view-cell mui-input-row"><label>地址：</label>
								<textarea name="receiver-add" placeholder="详细收货地址，如街道、楼号等" maxlength="30"></textarea></li>
							<li class="mui-table-view-cell mui-input-row"><label>电话：</label>
								<input type="number" value="" name="receiver-mobile" placeholder="建议填写常用的移动电话号码"></li>
			...
			<div id="main-addmanager" class="mui-scroll-wrapper">
				<div class="mui-content mui-scroll">
					...
					<c:forEach items="${AddressList }" var="item">
						<div class="mui-card">
							<input type="hidden" name="add" value="${item.addressId }" /> <input
								type="hidden" name="areaId"
								value="${item.provinceId },${item.cityId },${item.areaId },${item.streetId }" />
							<div class="mui-card-header mui-card-media">
								<span class="mui-pull-left">姓名：<b class="name">${item.realName }</b></span>
								<span class="mui-pull-right">手机：<b class="phone">${item.mobile }</b></span>
							</div>
							<div class="mui-card-content">
								<div class="mui-card-content-inner">
									<p>
										地址：<span class="add">${item.provinceName} ${item.cityName } ${item.areaName } ${item.streetName }</span> <span id="context">${item.address }</span>
									</p>
								</div>
							</div>
							<div class="mui-card-footer mui-radio mui-left">
								...
								<c:choose>
									<c:when test="${item.isDefault==false }">
	...
	<script type="text/javascript" charset="utf-8"> 			
			(function($, doc) {
				...
				$.ready(function() {
					...
					mui(document).on("tap", "#saveEdit", function(e) {
						...
						postadd(_type,_name,_add,_mobile,_areaid,_addid);
					});
					...
					function formCheck() {
						...
						if(_add != null && _add != "" && _add.length < 31 && !special.test(_add)) {
							check = true;
						} else {
					...
					//提交地址信息事件
					function postadd(type, name, add, mobile, areaId, addId) {
						formCheck();
						if(type == 0 && check) { //如果类型是"修改"，则修改目标元素中的文字
							...
							mui.ajax('${ctx}/wechat/user/addAddress.htm',{
								...
								success:function(data){
									...
									if(data.flag){
										mui.toast("新增成功");
										var cardText ='<div class="mui-card">'+
												    '<input type="hidden" name="add" value="'+data.message+'"/>'+
												    '<input type="hidden" name="areaId" value="'+areaId+'"/>'+
												    '<div class="mui-card-header mui-card-media">'+
													'<span class="mui-pull-left">姓名：<b class="name">'+name+'</b></span>'+
													'<span class="mui-pull-right">手机：<b class="phone">'+mobile+'</b></span>'+
												    '</div><div class="mui-card-content"><div class="mui-card-content-inner">'+
												    '<p>地址：<span class="add">'+document.getElementById("AreaName").innerHTML+
												    ' </span> <span id="context"> '+add+'</span></p></div></div>'+
												    '<div class="mui-card-footer mui-radio mui-left">'+
												    '<label for="r2">设为默认</label><input name="setAdd" type="radio" id="r2">'+	
													'<div class="button-group">'+
													'<button class="editor-btn"><span class="mui-icon mui-icon-compose"></span>编辑</button>'+
													'<button class="delete-btn"><span class="mui-icon mui-icon-trash"></span>删除</button>'+
													'</div></div></div>';
										var card = document.createElement("div");	
										card.innerHTML = cardText;
										document.querySelector('#main-addmanager .mui-content').appendChild(card);
										offCanvasWrapper.offCanvas('close');
										return ;
									}
									mui.toast(data.message);
					...
					mui('.mui-indexed-list-inner').on('tap', 'li.mui-indexed-list-item', function() {
						...
						levelText = String(name).replace(/,/g, " ");
						offCanvasWrapper_2.offCanvas('close');
						areaId.value = addId;
			...
			function getAddList() {
				...
				mui('#status-bar-right')[0].innerHTML = "选择街道";
				mui("#loading")[0].style.display = "inline-block";
				mui.ajax("${ctx }/wechat/shop/getCanbeDeliveryArea.htm", {
					data: {
						"key" : "59c23bdde5603ef993cf03fe64e448f1"
					},
					...
					success: function(data) {
						...
						mui.each(data.sort,function(index,value){
							...
							mui.each(data.message,function(index,item){
								if(value == item.flag){
									html += '<li data-level="' + item.level + '" data-name="'+item.allname+'" data-value="' + item.flag + '" data-id="' + item.id + '" class="mui-table-view-cell mui-indexed-list-item">'
									html += item.name
								}
							});
							html += '</li>'
