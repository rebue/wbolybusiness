					...
					<div class="mui-scroll">
						...
						<ul class="mui-table-view mui-input-group">
							...
							<li class="mui-table-view-cell mui-input-row"><label>收货人：</label>
								...
								placeholder="建议填写真实姓名"></li>
							<li class="mui-table-view-cell mui-input-row"><a
								class="mui-navigate-right" id="AreaPicker"> <label>区域：</label>
									<span class="area-text" id="AreaName"></span>
							</a></li>
							<li class="mui-table-view-cell mui-input-row"><label>地址：</label>
								<textarea name="receiver-add" placeholder="详细收货地址，如街道、楼号等"
									maxlength="30"></textarea></li>
							<li class="mui-table-view-cell mui-input-row"><label>邮编：</label>
								<input type="number" value="" name="postal-code"
								placeholder="必填" maxlength="6"
								onKeyUp="value=value.replace(/[^\d]/g,'')"></li>
							<li class="mui-table-view-cell mui-input-row"><label>电话：</label>
								<input type="number" value="" name="receiver-mobile"
								placeholder="建议填写常用的移动电话号码" maxlength="11"
			...
			<div id="main-addmanager" class="mui-scroll-wrapper">
				<div class="mui-content mui-scroll">
					...
					<c:forEach items="${AddressList }" var="item">
						<div class="mui-card">
							<input type="hidden" name="add" value="${item.id }" /> <input
								type="hidden" name="postalCode"
								value="${item.receiverPostCode }" /> <input type="hidden"
								name="areaId"
								value="${item.receiverProvince },${item.receiverCity },${item.receiverExpArea },${item.receiverExpArea }" />
							...
							<div class="mui-card-content">
								<div class="mui-card-content-inner">
									<p>
										地址： <span class="add">${item.receiverProvince}
											${item.receiverCity } ${item.receiverExpArea }</span> <span
											id="context">${item.receiverAddress }</span>
									</p>
								</div>
							</div>
	...
	<script type="text/javascript" charset="utf-8">
		(function($, doc) {
			...
			$.ready(function() {
						...
						mui('#main-addmanager.mui-scroll-wrapper').scroll({
							bounce : false
						});
						...
						function formCheck() {
							...
							if (_add != "" && _add != null && _add.length < 31
									&& !special.test(_add)) {
								check = true;
							} else {
						...
						//提交地址信息事件
						function postadd(type, name, add, mobile, areaId,
								addId, postalCode) {
						...
						mui(document).on("change", ".mui-card input[type='radio']", function(e) {
											...
											if (this.checked == true) {
												var _addid = this.parentNode.parentNode
														.querySelector("input[name='add']").value;//地址标识
												if (_addid == null
														|| _addid == "") {
													mui.toast("请求参数有误");
													return;
						...
						//收起侧栏
						mui(document).on("tap", "#action_back_side",
								function(e) {
									offCanvasWrapper.offCanvas('close');
								});
						mui(document).on("tap", "#action_back_side_2",
								function(e) {
						...
						mui(document)
								.on(
										"tap",
										".mui-card button.delete-btn",
										function(e) {
											var id = this.parentNode.parentNode.parentNode
													.querySelector("input[name='add']").value;//地址标识
											var elem = this;
											mui
													.confirm(
															...
															function(e) {
																if (e.index == 0) {
																	...
																	mui
																			.ajax(
																					'${ctx}/wechat/user/delAddress.htm',
																					{
																						data : {
																							key : '59c23bdde5603ef993cf03fe64e448f1',
																							addressId : id
								...
								.addEventListener(
										...
										function(event) {
											offCanvasWrapper_2
													.offCanvas('show');
											var header = document
													.querySelector('header#list-head');
											var list = document
													.getElementById('list');
											list.style.height = (startHeight - header.offsetHeight)
													+ 'px';
											getAddList(0);
										});

						mui('.mui-indexed-list-inner')
								.on(
										'tap',
										'li.mui-indexed-list-item',
										function() {
											var addId = this
													.getAttribute("data-id");
											var index = addId.lastIndexOf(",");
											var str = addId.substring(
													index + 1, addId.length);
											var level = this
													.getAttribute("data-level");
											var addName = this.innerText;
											var index = level - 1;
											if (str.length < 12) {
												getAddList(level, addId,
														addName);
								...
								.on(
										...
										function() {
											...
											var level = this
													.getAttribute("data-level");
											var active = document
													.getElementById('status-bar-right');
											if (str.length < 12) {
												getAddList(parseInt(level)
														- parseInt(1), str);
											}
											;
											switch (level) {
		...
		function getAddList(level, id, name) {
			...
			mui
					.ajax(
							"${ctx }/wechat/shop/effectiveArea.htm",
							{
								data : {
									"id" : id, // 所点击条目的编号
								...
								success : function(data) {
									...
									mui
											.each(
													...
													function(index, value) {
														html += '<li data-group="' + value + '" class="mui-table-view-divider mui-indexed-list-group">'
																+ value
																+ '</li>'
														mui
																.each(
									...
									if (name != null) {
										var newItem = document
												.createElement("a")
										var parent = document
												.querySelector('#status-bar-left');
										var active = document
												.getElementById('status-bar-right');
										newItem.innerHTML = name;
										parent.appendChild(newItem);
										newItem.setAttribute("data-level",
												level);
