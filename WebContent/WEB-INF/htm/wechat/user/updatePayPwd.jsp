...
<html>
	...
	<script type="text/javascript">
		$(function () {
			$.ajax({
				url : "${ctx }/wechat/user/payPwIsExis.htm?userId=${userId}",
				type : "get",
			...
			$(document).on("tap", "#updateSubmit", function() {
			    ...
			    console.log("测试成功");
				$.ajax({
					url : "${ctx}/wechat/user/changePayPassword.htm",
					type : "post",
					dataType:'json',//服务器返回json格式数据
					data : {oldPayPswd : encodeURIComponent(CryptoJS.MD5(updateOldPassword)), newPayPswd : encodeURIComponent(CryptoJS.MD5(updateValNewPassword))},
					success:function(data){
						if (data.result == 1) {
							mui.toast(data.msg);
			...
			$(document).on("tap", "#setSubmit", function() {
				...
				console.log("测试成功");
				$.ajax({
					url : "${ctx}/wechat/user/setPayPassword.htm",
					type : "post",
					dataType:'json',//服务器返回json格式数据
					data : {newPayPswd : encodeURIComponent(CryptoJS.MD5(setValNewPassword))},
					success:function(data){
						if (data.result == 1) {
							mui.toast(data.msg);
							window.location.href = "${ctx }/wechat/user/userCenter.htm?promoterId=${userId}";
	...
	<body>
		<header class="mui-bar mui-bar-nav" style="box-shadow: none;">
			...
			<h4 class="mui-title">修改或设置支付密码</h4>
		</header>
		<div id="wallet_main">
			<div class="mui-content">
				<div class="mui-slider tab-slider">
					<div id="sliderSegmentedControl"
						...
						<div class="mui-control-item mui-active" id="payTreasure">修改支付密码</div>
						<div class="mui-control-item" id="bankCard">设置支付密码</div>
					</div>
				</div>
				<div id='login-form' class="divTop1">
					...
					<div class="mui-input-row">
						...
						<p style="color: red;">该密码可用于支付操作，请务必牢记！</p>
					</div>
